...
class GitHubRepoTeamAssigner:
    def __init__(self):
        ...
        logger.info(f"API URL: {self.api_url} | Org: {self.org_name} | CSV: {self.input_csv}")
    ...
    def get_team_id(self, team_slug):
        url = f"{self.api_url}/orgs/{self.org_name}/teams/{team_slug}"
    ...
    def check_repo_exists(self, repo_name):
        url = f"{self.api_url}/repos/{self.org_name}/{repo_name}"
        r = self.make_api_request("GET", url)
        if r.status_code == 200:
            return True
        elif r.status_code == 404:
    ...
    def add_repo_to_team(self, team_slug, repo_name, permission='pull'):
        url = f"{self.api_url}/orgs/{self.org_name}/teams/{team_slug}/repos/{self.org_name}/{repo_name}"
