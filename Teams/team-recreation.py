...
class GitHubTeamRecreationFetcher:
    def __init__(self):
        ...
        logger.info(f"Initialized GitHub Team Recreation Fetcher for org: {self.org_name}, input CSV: {self.input_csv}, API URL: {self.api_url}")
    ...
    def create_team(self, team_name, team_slug, description="", privacy="closed", parent_team_id=None):
        ...
        resp = self.make_api_request("POST", f"{self.api_url}/orgs/{self.org_name}/teams", json=payload)
    ...
    def add_member_to_team(self, team_slug, username, role="member"):
        resp = self.make_api_request("PUT", f"{self.api_url}/orgs/{self.org_name}/teams/{team_slug}/memberships/{username}",
                            json={"role": role})
    ...
    def check_team_exists(self, team_name):
        page = 1
        while True:
            resp = self.make_api_request("GET", f"{self.api_url}/orgs/{self.org_name}/teams",
    ...
    def test_github_connection(self):
        user_resp = self.make_api_request("GET", f"{self.api_url}/user")
        if user_resp.status_code != 200:
            logger.error(f"[ERROR] Auth failed: {user_resp.status_code} - {user_resp.text}")
            return False
        org_resp = self.make_api_request("GET", f"{self.api_url}/orgs/{self.org_name}")
